...
const state = ref<{ count: number; history: number[] }>()
...
This step follows what's shown in [basic-setup](../basic-setup.md) for a more documented guide go check it out.

App.vue
```vue
<script setup lang="ts">
import { TanStackDevtools } from '@tanstack/vue-devtools'
import DevtoolPanel from './DevtoolPanel.vue'
...
<template>
  <TanStackDevtools
    :event-bus-config="{ debug: true }"
    :plugins="[
      {
        name: 'Custom devtools',
        component: DevtoolPanel,
      },
    ]"
  />
</template>

...
// With Cloudflare Workers, this runs in an isolated environment.
...
const greet = createServerFn({ method: 'GET' }).handler(async () => {
  const message = `Hello from Cloudflare Worker at ${new Date().toLocaleTimeString()}`
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 20px' }}>
      ...
      <p style={{ color: '#666', marginBottom: '32px' }}>
        Each button calls a server function running in Cloudflare Workers'
        isolated environment. Open the TanStack Devtools panel (bottom-right)
        and switch to the "Server Events" tab to see events arriving from the
        server.
      </p>

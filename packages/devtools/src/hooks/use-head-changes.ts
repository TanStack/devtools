...
export type HeadChange =
  ...
  | { kind: "attr"; target: Element; name: string | null; oldValue: string | null }
    ...
    const headObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.type === "childList") {
          ...
          m.removedNodes.forEach((node) => onChange({ kind: "removed", node }, m));
        } else if (m.type === "attributes") {
          const el = m.target as Element;
          onChange(
            { kind: "attr", target: el, name: m.attributeName, oldValue: m.oldValue ?? null },
            m
          );
        } else {
